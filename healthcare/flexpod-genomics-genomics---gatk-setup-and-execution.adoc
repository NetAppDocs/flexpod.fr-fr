...
image:flexpod-genomics-image8.png["Un diagramme illustrant le flux de travail en génomique, depuis le prélèvement d'échantillons jusqu'aux modèles prédictifs, en passant par le séquençage, l'analyse des variants et l'intégration aux dossiers médicaux électroniques."]
...
+
image:flexpod-genomics-image10.png["Un tableau des réservations d'adresses IP VLAN indiquant les plages, les masques de sous-réseau, ainsi que les usages pour la gestion, les VM, NFS et iSCSI"]

. Configuration des LUN de démarrage iSCSI sur le SVM ONTAP
+
image:flexpod-genomics-image9.png["Capture d'écran d'ONTAP System Manager montrant six LUN de démarrage ESXi dans la machine virtuelle de stockage Healthcare_SVM"]

. Mapper les LUN sur des groupes initiateurs iSCSI.
+
image:flexpod-genomics-image11.png["Capture d'écran montrant ESXi_Boot_Lun_1 mappé au groupe d'initiateurs iSCSI GenomicsESXi_1"]

+
image:flexpod-genomics-image12.png["Capture d'écran d'ONTAP System Manager affichant les détails du LUN de démarrage ESXi mappé au groupe d'initiateurs GenomicsESXi_2"]

. Installez vSphere 7.0 avec le démarrage iSCSI.
. Enregistrez les hôtes ESXi avec vCenter.
+
image:flexpod-genomics-image13.png["Capture d'écran de l'inventaire du client vSphere montrant le dossier Genomics avec les hôtes ESXi et les machines virtuelles"]

. Provisionner un datastore NFS `infra_datastore_nfs` Sur le stockage ONTAP.
+
image:flexpod-genomics-image14.png["Capture d'écran de ONTAP System Manager montrant le volume NFS Healthcare_Workload_NFS_Vol sélectionné en ligne"]

. Ajoutez le datastore au vCenter.
+
image:flexpod-genomics-image16.png["Capture d'écran du vSphere Client affichant le résumé du datastore NFS 3 infra_datastore_nfs"]

. À l'aide de vCenter, ajoutez un datastore NFS aux hôtes ESXi.
+
image:flexpod-genomics-image15.png["Capture d'écran du vSphere Client montrant les hôtes infra_datastore_nfs connectés avec un état normal"]
...
+
image:flexpod-genomics-image17.png["Capture d'écran d'un terminal Linux montrant /dev/sdb monté sur /mnt/genomics avec une utilisation de 2 %"]
