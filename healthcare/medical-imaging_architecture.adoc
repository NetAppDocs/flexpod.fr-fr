...
image:medical-imaging_image3.png["Un schéma illustrant l'infrastructure d'imagerie médicale FlexPod avec les commutateurs UCS, Nexus, AFF et MDS"]
...
image:medical-imaging_image4.png["Un diagramme illustrant les serveurs SnapCenter, l'intégration VMware, les référentiels de métadonnées répliqués et les plug-ins d'application"]

NetApp MetroCluster est utilisé par des milliers d'entreprises dans le monde pour garantir une haute disponibilité (HA), une perte de données nulle et des opérations ininterrompues, aussi bien au sein qu'en dehors du centre de données. MetroCluster est une fonctionnalité gratuite du logiciel ONTAP qui réplique de manière synchrone les données et la configuration entre deux clusters ONTAP situés sur des sites distincts ou dans des domaines de panne différents. MetroCluster assure une disponibilité continue du stockage pour les applications en gérant automatiquement deux objectifs : un objectif de point de récupération (RPO) nul grâce à la réplication synchrone des données écrites sur le cluster. Un objectif de temps de récupération (RTO) quasi nul grâce à la réplication de la configuration et à l'automatisation de l'accès aux données sur le second site. MetroCluster offre une simplicité grâce à la réplication automatique des données et de la configuration entre les deux clusters indépendants situés sur les deux sites. Lorsque du stockage est provisionné dans un cluster, il est automatiquement répliqué sur le second cluster du second site. NetApp SyncMirror fournit une copie complète de toutes les données avec un RPO nul. Par conséquent, les charges de travail d'un site peuvent basculer à tout moment vers l'autre site et continuer à fournir des données sans perte de données. Pour plus d'informations, consultez la documentation https://fieldportal.netapp.com/s/article/tr4705-netapp-metrocluster-solution-architecture-and-design-1693982882873["ici"^].
...
image:medical-imaging_image5.png["Un schéma illustrant une clinique communautaire connectée aux systèmes hospitaliers via un bus de services d'entreprise"]
...
Les systèmes d'imagerie médicale commencent à utiliser  https://www.netapp.com/pdf.html?item=/media/7398-sb-flexpod-datacenter-ai.pdf["Capacités de calcul basées sur les GPU"^] pour optimiser les flux de travail humains grâce au prétraitement des images, améliorant ainsi l'efficacité. Les systèmes d'imagerie médicale d'entreprise typiques tirent parti des capacités d'efficacité de stockage de pointe de NetApp. Les systèmes d'imagerie médicale d'entreprise utilisent généralement RMAN pour les activités de sauvegarde, de récupération et de restauration. Pour de meilleures performances et pour réduire le temps nécessaire à la création des sauvegardes, la technologie Snapshot est disponible pour les opérations de sauvegarde et la technologie SnapMirror est disponible pour la réplication.

La figure ci-dessous présente les composants d'application logique dans une vue architecturale superposée.

image:medical-imaging_image6.png["Un schéma illustrant les composants d'imagerie médicale superposés sur FlexPod avec une reprise après sinistre répliquée"]

La figure ci-dessous présente les composants de l'application physique.

image:medical-imaging_image7.png["Un schéma qui montre des centres de données FlexPod actifs et de secours avec le stockage NetApp ONTAP et des serveurs d’imagerie médicale"]
