...
* *Protection contre les ransomwares.*  La protection contre les ransomwares de NetApp Console analyse les sources de données dans les environnements sur site et cloud, détecte les vulnérabilités de sécurité et fournit leur état de sécurité actuel ainsi qu'une évaluation des risques.  Il fournit ensuite des recommandations concrètes que vous pouvez approfondir et suivre pour remédier à la situation.  Cela vous permet de protéger vos données de santé critiques contre les attaques de ransomware.
...
Cette section décrit la topologie logique de la solution.  La figure suivante représente la topologie de la solution composée de l'environnement sur site FlexPod , de NetApp Cloud Volumes ONTAP (CVO) exécuté sur Amazon Web Services (AWS) et de la plateforme SaaS NetApp Console .

image:fhc-cvoe-image2.png["Cette image illustre la réplication SnapMirror entre une instance FlexPod exécutant ONTAP et NetApp Cloud Volumes ONTAP exécuté dans le cloud public."]
