...
[role="lead"]
FabricPool nécessite ONTAP 9.2 ou une version ultérieure. Les exigences supplémentaires de FabricPool dépendent de la version d'ONTAP utilisée et du niveau cloud associé. Dans les versions antérieures à ONTAP 9.8, FabricPool est uniquement pris en charge sur les niveaux locaux SSD. La prise en charge des agrégats HDD a été ajoutée dans ONTAP 9.8. Pour les plateformes AFF d'entrée de gamme à capacité fixe et relativement faible, telles que le NetApp AFF C190, FabricPool peut s'avérer très efficace pour déplacer les données inactives vers le niveau cloud.
...
* NetApp AFF
* NetApp FAS FabricPool est pris en charge sur toutes les plateformes capables d'exécuter ONTAP 9.2, à l'exception des suivantes :
+
** FAS8020
...
NOTE: Veuillez vous référer à la  https://hwu.netapp.com/DetailViews/Home.aspx["NetApp Hardware Universe"^] pour les informations les plus récentes sur les plateformes prises en charge avec FabricPool.

* NetApp ONTAP Select
+

NOTE: NetApp recommande d'utiliser des niveaux locaux FabricPool entièrement SSD.
...
** NetApp StorageGRID 10.3 et versions ultérieures
+

NOTE: Glacier Flexible Retrieval et Glacier Deep Archive ne sont pas pris en charge.
...
** Azure NetApp Files
** NetApp Cloud Volumes pour Google Cloud
** NetApp Cloud Volumes ONTAP pour AWS
** NetApp Cloud Volumes ONTAP pour Azure
...
image:cloud-fabricpool_image6.png["Un schéma montrant deux contrôleurs NetApp AFF A300 connectant des LIF inter-clusters à AWS S3"]
...
image:cloud-fabricpool_image7.png["Capture d'écran des résultats du profileur de magasin d'objets ONTAP CLI montrant la latence et le débit des opérations PUT et GET"]
...
FabricPool licenses (y compris la capacité supplémentaire pour les licences existantes) peuvent être achetées par incréments de 1 To.
