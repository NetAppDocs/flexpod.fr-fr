...
FlexPod est certifié EHR et prend en charge une plateforme contenant Cisco UCS avec des processeurs Intel Xeon, Red Hat Enterprise Linux (RHEL) et la virtualisation avec VMware ESXi.  Cette plateforme, associée au classement de niveau de confort élevé d'EHR pour le stockage NetApp exécutant ONTAP, vous permet d'exécuter vos applications de soins de santé dans un cloud privé entièrement géré via FlexPod , qui peut également être connecté à n'importe quel fournisseur de cloud public.



== NetApp Console

NetApp Console est une plateforme de gestion SaaS de classe entreprise qui permet aux experts informatiques et aux architectes cloud de gérer de manière centralisée leur infrastructure multicloud hybride à l'aide des solutions cloud NetApp .  Il offre un système centralisé pour visualiser et gérer votre stockage sur site et dans le cloud, prenant en charge les environnements hybrides, les fournisseurs de cloud multiples et les comptes associés. Pour plus d'informations, voir link:https://docs.netapp.com/us-en/console-family/index.html["Documentation de la NetApp Console"^].



== Agent de console

Une instance d'agent Console permet à la Console de gérer les ressources et les processus au sein d'un environnement de cloud public.  Un agent Console est nécessaire pour de nombreuses fonctionnalités offertes par la Console, et il peut être déployé dans le cloud ou sur le réseau local.

Un agent Console est pris en charge aux emplacements suivants :
...
link:https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["En savoir plus sur les agents de console"^].
...
Pour plus d'informations, consultezlink:https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/["Cloud Volumes ONTAP"^] .
...
Pour plus d'informations, voir link:https://docs.netapp.com/us-en/active-iq-unified-manager/["Active IQ Unified Manager"^].
...
Pour plus d'informations sur VMware vSphere et ses composants, consultezlink:https://www.vmware.com/products/vsphere.html["VMware vSphere"^] .
...
Pour plus d'informations, voirlink:https://www.vmware.com/products/vcenter.html["VMware vCenter"^] .



== Révisions matérielles et logicielles

Cette solution de cloud hybride peut être étendue à tout environnement FlexPod exécutant des versions prises en charge de logiciels, de microprogrammes et de matériels, comme défini dans les link:http://mysupport.netapp.com/matrix["Matrice d'interopérabilité NetApp"^], link:https://ucshcltool.cloudapps.cisco.com/public/["Compatibilité matérielle et logicielle UCS"^] et link:https://www.vmware.com/resources/compatibility/search.php["Guide de compatibilité VMware"^].
...
|===
Le tableau suivant présente les versions de Console et de Cloud Volumes ONTAP .
